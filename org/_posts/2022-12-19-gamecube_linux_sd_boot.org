...
- *USB Gecko* is probably the closest to a serial connection we have. It's a small device that plugs into a memory card slot, and a USB cable then connects the [[https://wiibrew.org/wiki/USB_Gecko][USB Gecko]] to your computer. [[https://wiibrew.org/wiki/Debugging][The main use case for USB Gecko is debugging your applications using GDB]], but thanks to having a simple serial protocol, we can configure it for that need as well. We will look at that more closely [[target][in the section on booting]]. Not really easy to get anymore, but there is [[https://www.tindie.com/products/webhdx/usb-gecko-debugging-tool-for-gamecubewii/][a seller on Tindie now selling them]].
